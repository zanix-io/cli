...
Deno.test(
  'zanix space build: compiles a configured messagesDir to AST under {outDir}/messages/0/... — ' +
    'the live messagesDir source is NEVER touched',
  async () => {
    await withMessagesProject(
      ...
      async (root) => {
        ...
        // The compiled destination — root index 0, since `messagesDir` here is a single string,
        // not an array.
        const compiledPath = join(root, '.dist', 'client', 'messages', '0', 'en', 'index.json')
        const compiled = JSON.parse(await Deno.readTextFile(compiledPath))
        ...
        // The developer's own hand-authored ICU source — untouched, still readable as plain
        // strings. This is the real regression check for the in-place-overwrite bug this fixed.
        const sourcePath = join(root, 'messages', 'en', 'index.json')
        const source = JSON.parse(await Deno.readTextFile(sourcePath))
        assertEquals(source, { title: 'Welcome', greet: 'Hello, {name}!' })
      },
    )
  },
)
...
Deno.test(
  ...
  async () => {
    await withMessagesProject({ home: { title: 'Welcome' } }, async (root) => {
      const command = registerCommand()
      await command.settings.actionHandler({ messages: false })

      const sourcePath = join(root, 'messages', 'en', 'index.json')
      const raw = JSON.parse(await Deno.readTextFile(sourcePath))
      assertEquals(raw, { title: 'Welcome' })

      const compiledDirExists = await Deno.stat(join(root, '.dist', 'client', 'messages')).then(
        () => true,
      ).catch(() => false)
      assert(
        !compiledDirExists,
        'expected no compiled messages output when --no-messages is passed',
      )
    })
  },
)
