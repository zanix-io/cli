...
async function spaceDevAction(
  ...
  try {
    ...
    if (options.graphqlCheck !== false) {
      // A RELATIVE specifier, never the bare import-map alias (`commands/space/shared/...`) —
      // same reasoning as `command.ts`'s own `SPACE_DEV_ACTION_SPECIFIER`: a bare alias only
      // resolves via this file's nearest `deno.jsonc` "imports" entry when loaded from a real
      // local `file://` checkout, not once this module loads from a remote `jsr:` specifier
      // (`deno install -g`'s own generated shim config carries no import map at all) — see
      // `../build/action.ts`'s own identical note for the full account.
