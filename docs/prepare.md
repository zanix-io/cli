...
| `-p, --project-type <type>` | `'library'`, `'space-server'`, `'space'`, `'server'`, or `'app'` — used when generating the publish workflow AND the Dockerfile (only `'library'`/`'app'` produce a publish workflow; every type but `'library'` produces a Dockerfile). |
...
- **Git hooks** (`--hooks-engine native`, the default):
  ...
  - `pre-push` — runs `deno test --allow-all`; if a test contains
    ...
    `--no-verify`).
- **`.github/workflows/publish.yml`** — runs `deno test --allow-all` then
  `deno publish` on every push/PR to the main branch.
